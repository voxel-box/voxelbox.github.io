...
/* =====================================================================
   FLOATING IN PRODUCTION — live portfolio feed → home cards
   Mirrors demos.voxelbox.org/portfolio.json so any site added to the
   portfolio auto-appears here (newest first). Falls back to the static
   cards baked into index.html if the feed is unreachable.
   ===================================================================== */
const PORTFOLIO_URL = "https://demos.voxelbox.org/portfolio.json";
const HOLO_MAX = 8;
const escq = (s)=>String(s==null?"":s).replace(/[&<>"']/g,(m)=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]));

const HOLO_ART = [
  `<svg viewBox="0 0 400 190" preserveAspectRatio="xMidYMid slice"><rect width="400" height="190" fill="#0d0f14"/><circle cx="320" cy="46" r="130" fill="#ff7a1a" opacity=".08"/><g stroke="#2a3140" stroke-width="1"><path d="M0 150h400M0 120h400M0 90h400"/></g><rect x="40" y="52" width="120" height="10" rx="2" fill="#f2f4f8" opacity=".8"/><rect x="40" y="72" width="86" height="8" rx="2" fill="#9aa3b2" opacity=".5"/><rect x="40" y="96" width="52" height="16" rx="4" fill="#ff7a1a" opacity=".85"/><g fill="#ff7a1a"><rect x="250" y="60" width="26" height="26" opacity=".9"/><rect x="280" y="88" width="26" height="26" opacity=".5"/><rect x="222" y="88" width="26" height="26" opacity=".35"/></g></svg>`,
  `<svg viewBox="0 0 400 190" preserveAspectRatio="xMidYMid slice"><rect width="400" height="190" fill="#0c0e14"/><circle cx="80" cy="150" r="120" fill="#5b7cff" opacity=".09"/><g fill="#5b7cff"><rect x="40" y="120" width="18" height="40" opacity=".8"/><rect x="66" y="96" width="18" height="64" opacity=".55"/><rect x="92" y="76" width="18" height="84" opacity=".9"/><rect x="118" y="106" width="18" height="54" opacity=".4"/></g><path d="M180 120l40-30 34 18 52-44 54 26" stroke="#ff7a1a" stroke-width="2.5" fill="none"/><g fill="#ff7a1a"><circle cx="220" cy="90" r="4"/><circle cx="306" cy="64" r="4"/></g></svg>`,
  `<svg viewBox="0 0 400 190" preserveAspectRatio="xMidYMid slice"><rect width="400" height="190" fill="#0d0d15"/><circle cx="200" cy="95" r="120" fill="#8b5cf6" opacity=".08"/><g transform="translate(200,95)" stroke="#8b5cf6" fill="none" stroke-width="1.4" opacity=".5"><ellipse rx="90" ry="34"/><ellipse rx="90" ry="34" transform="rotate(60)"/><ellipse rx="90" ry="34" transform="rotate(-60)"/></g><circle cx="200" cy="95" r="16" fill="#ff7a1a" opacity=".9"/><g fill="#8b5cf6"><circle cx="290" cy="95" r="6"/><circle cx="155" cy="24" r="6"/><circle cx="155" cy="166" r="6"/></g></svg>`,
  `<svg viewBox="0 0 400 190" preserveAspectRatio="xMidYMid slice"><rect width="400" height="190" fill="#0b1013"/><circle cx="320" cy="150" r="130" fill="#4fd6c0" opacity=".08"/><g fill="#4fd6c0"><rect x="60" y="120" width="22" height="22" opacity=".3"/><rect x="82" y="98" width="22" height="22" opacity=".5"/><rect x="104" y="76" width="22" height="22" opacity=".8"/><rect x="126" y="98" width="22" height="22" opacity=".5"/><rect x="148" y="120" width="22" height="22" opacity=".3"/></g><rect x="104" y="48" width="22" height="22" fill="#ff7a1a" opacity=".9"/><g fill="#9aa3b2" opacity=".7"><rect x="250" y="70" width="110" height="8" rx="2"/><rect x="250" y="88" width="80" height="8" rx="2" opacity=".6"/></g></svg>`,
  `<svg viewBox="0 0 400 190" preserveAspectRatio="xMidYMid slice"><rect width="400" height="190" fill="#0b120d"/><circle cx="300" cy="40" r="130" fill="#67d073" opacity=".08"/><g stroke="#1d2a20" stroke-width="1"><path d="M0 60h400M0 100h400M0 140h400"/></g><g fill="#67d073"><rect x="60" y="120" width="30" height="40" opacity=".5"/><rect x="110" y="96" width="30" height="64" opacity=".7"/><rect x="160" y="70" width="30" height="90" opacity=".9"/></g><path d="M60 120l50-24 50-26 60-18" stroke="#ff7a1a" stroke-width="2.5" fill="none"/><circle cx="280" cy="52" r="5" fill="#ff7a1a"/></svg>`,
  `<svg viewBox="0 0 400 190" preserveAspectRatio="xMidYMid slice"><rect width="400" height="190" fill="#0e0d0a"/><circle cx="90" cy="60" r="130" fill="#ffce4d" opacity=".07"/><path d="M0 100h90l16-46 26 92 20-64 18 40h210" stroke="#ffce4d" stroke-width="2.5" fill="none"/><g fill="#ff7a1a"><circle cx="132" cy="146" r="4"/><circle cx="152" cy="82" r="4"/></g><g fill="#9aa3b2" opacity=".55"><rect x="250" y="60" width="100" height="8" rx="2"/><rect x="250" y="78" width="70" height="8" rx="2" opacity=".6"/><rect x="250" y="96" width="84" height="8" rx="2" opacity=".4"/></g></svg>`,
];

function holoCard(s, i){
  const parts = String(s.title||"").split(/\s[–—-]\s/);
  const name = (parts[0] || s.slug || "Live site").trim();
  const blurb = (parts.slice(1).join(" — ").trim()) || s.description || "";
  const tags = (s.tags||[]).slice(0,3).map((t)=>`<span class="tag">${escq(t)}</span>`).join("");
  return `<a class="holo" href="${escq(s.url)}" target="_blank" rel="noopener noreferrer" data-reveal data-delay="${(i%4)*0.08}s" style="--f:${(i*0.6).toFixed(1)}s">
        <span class="holo-open">visit →</span>
        <div class="holo-art" aria-hidden="true">${HOLO_ART[i % HOLO_ART.length]}</div>
        <div class="holo-cap">
          <h3>${escq(name)}</h3>
          <p>${escq(blurb)}</p>
          ${tags ? `<div class="holo-tags">${tags}</div>` : ""}
        </div>
      </a>`;
}

function initHolos(){
  const grid=$("[data-holo-grid]");
  if(!grid) return;
  fetch(PORTFOLIO_URL,{cache:"no-store"}).then((r)=>r.ok?r.json():Promise.reject()).then((d)=>{
    ...
    grid.innerHTML=sites.map(holoCard).join("");
    const cards=$$("[data-reveal]",grid);
    if(REDUCED){ cards.forEach((el)=>el.classList.add("vis")); return; }
    cards.forEach((el,i)=>{
      if(el.dataset.delay) el.style.setProperty("--d",el.dataset.delay);
      setTimeout(()=>el.classList.add("vis"), 80+i*70);
    });
